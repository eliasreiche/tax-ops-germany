...
zuordnung-Fix): eine binäre Mandats-Datei unter `--datei` darf keinen
Traceback erzeugen, sondern muss als sauberer Eingabefehler (Exit 2) enden.
`core/context/validator.py` --datei geht über `pruefe_mandat_datei()` in
`schema.py`, die `UnicodeDecodeError` beim Lesen in `KontextEingabeFehler`
übersetzt.
...
def test_cli_binaere_mandatsdatei_exit2_kein_traceback(tmp_path):
    datei = tmp_path / "2026-001.md"
    datei.write_bytes(_PNG_BYTES)

    ergebnis = lauf(VALIDATOR, "--datei", datei)
